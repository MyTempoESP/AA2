...
void memstat() {
  log("MEM=$");
  logx(N4_DIC_SZ + N4_STK_SZ + N4_TIB_SZ); // forth memory block
  log("[DIC=$");
  logx(N4_DIC_SZ); // dictionary size
  log("|STK=$");
  logx(N4_STK_SZ); // stack size
  log("|TIB=$");
  logx(N4_TIB_SZ);
#if ARDUINO
  S16 bsz = (S16)((U8 *)&bsz - _tib); // free for TIB in bytes
  show("] auto=");
  d_num((U16)((U8 *)&bsz - &_tib[N4_TIB_SZ]));
#endif // ARDUINO
}
///@}
...
void d_name(U8 op, const char *lst, U8 space) {
  ...
  if ((c = pgm_read_byte(p + 1)) != ' ' || space)
    d_chr(c);
  if ((c = pgm_read_byte(p + 2)) != ' ' || space)
    d_chr(c);
}
///
    ...
    if (c == '\r' || c == '\n') { /// * split on RETURN
      ...
      *(--p) = ' ';
      ...
      d_chr('\b');
               ...
               (U8 *)(&c -
                      sizeof(
                          U32))) { /// * prevent buffer overrun (into auto vars)
      show("TIB!\n");
      *p = 0;
      break;
    } else
...
U8 *get_token(U8 rst) {
  ...
  while (_empty || *tp == 0 || *tp == '\\') {
    ...
    while (*tp == ' ')
      tp++; ///>  skip leading spaces
  }
  if (!dq) {
    while (*tp == '(' &&
           *(tp + 1) == ' ') { /// * handle ( ...) comment, TODO: multi-line
      while (*tp && *tp++ != ')')
        ; ///> find the end of comment
      while (*tp == ' ')
        tp++; ///> skip trailing spaces
    }
  }
  U8 *p = (U8 *)tp;
  ...
  while (*tp == ' ')
    tp++; /// * skip spaces, advance pointer to next token
  if (*tp == 0 || *tp == '\\') {
    tp = _tib;
    _empty = 1;
  }
