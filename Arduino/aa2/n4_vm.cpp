 ...
 *
 * #### Forth VM stack opcode macros (notes: rp grows upward and may collide
 * with sp)
 *
 * @code memory space
 *                                    SP0 (sp max to protect overwritten of vm
 * object) mem[...dic_sz...|...stk_sz...|......heap......]max |            | | |
 *           dic-->       +-->rp  sp<--+-->tib   auto<--+
 *                                TOS NOS
 * @endcode
 */
...
#define PUSH(v)                                                                \
  (push((S16)v)) /**< push v onto parameter stack         */
#define POP() (pop())  /**< pop value off parameter stack       */
#define RPUSH(a)                                                               \
...
///@{
#define DIC(n)                                                                 \
  ((U8 *)dic + (n)) /**< convert dictionary index to a memory pointer */
#define IDX(p)                                                                 \
  ((U16)((U8 *)(p) - dic)) /**< convert memory pointer to a dictionary index   \
                            */
///@}
namespace N4VM {
///
...
void _init() {
  show(APP_NAME);
  show(APP_VERSION); /// * show init prompt
...
void _dump(U16 p0, U16 sz0) {
  ...
  for (U16 i = 0; i < sz; i += DUMP_PER_LINE) {
    ...
    d_chr(' ');
    for (U8 j = 0; j < DUMP_PER_LINE;
         j++, p++) { // print and advance to next byte
      char c = *p & 0x7f;
...
void _immediate(U16 op) {
  ...
  case 0:
    N4Asm::compile(vm.rp);
    break; /// * : (COLON), switch into compile mode (for new word)
  case 1:
    N4Asm::variable();
    break; /// * VAR, create new variable
  case 2:
    N4Asm::constant(POP());
    break; /// * VAL, create new constant
  ///> interrupt handlers
  case 3:
    N4Intr::add_pcisr( /// * PCI, create a pin change interrupt handler
        POP(), N4Asm::query());
    break;
  case 4:       /// * TMI, create a timer interrupt handler
    op = POP(); ///< tmp = ISR slot#
    N4Intr::add_tmisr(op, POP(), N4Asm::query());
    break; /// * period in multiply of 10ms
  ///> numeric radix
  case 5:
    set_hex(1);
    break; /// * HEX
  ...
  case 7:
    N4Asm::forget();
    break; /// * FGT, rollback word created
  case 8:
    N4Asm::words();
    break; /// * WRD
  ...
  case 13:
    N4Asm::save(1);
    break; /// * SEX - save/execute (autorun)
#if ARDUINO
  case 14:
    _init();
    break; /// * BYE, restart
...
#define _X(i, g)                                                               \
  ...
  return
#define LL(i)                                                                  \
  &&L_##i##0, &&L_##i##1, &&L_##i##2, &&L_##i##3, &&L_##i##4, &&L_##i##5,      \
      &&L_##i##6, &&L_##i##7, &&L_##i##8, &&L_##i##9
  static void *vt[] = {// computed goto branching table
                       LL(),  LL(1),  LL(2),  LL(3),  LL(4),
                       LL(5), &&L_60, &&L_61, &&L_62, &&L_63};
#else // !N4_USE_GOTO
...
#define _X(i, g)                                                               \
  ...
  DISPATCH(op) {      // switch(op) or goto *vt[op]
    ...
    _X(50,
       N4Intr::enable_timer(POP())); // TME - enable/disable timer2 interrupt
    _X(51,
       N4Intr::enable_pci(POP())); // PCE - enable/disable pin change interrupts
    _X(52, NanoForth::call_api(POP())); // API
#if N4_DOES_META
    ///> meta programming (for advance users)
    _X(53, N4Asm::create()); // CRE, create a word (header only)
    _X(54,
       N4Asm::comma(POP())); // ,    comma, add a 16-bit value onto dictionary
    _X(55,
       N4Asm::ccomma(POP())); // C,   C-comma, add a 8-bit value onto dictionary
...
void _nest(U16 xt) {
  ...
  while (xt != LFA_END) { ///> walk through instruction sequences
...
#if TRC_LEVEL > 0
    if (trc)
      N4Asm::trace(xt, op); // execution tracing when enabled
#endif                      // TRC_LEVEL
    ...
    if ((op & CTL_BITS) == JMP_OPS) { ///> determine control bits
      ...
      case OP_CDJ:
        xt = POP() ? xt + 2 : w;
        break; // 0xd0 conditional jump
      case OP_UDJ:
        xt = w;
        break;                   // 0xe0 unconditional jump
...
void outer() {
  ...
  U16 tmp;
  switch (N4Asm::parse(tkn, &tmp,
                       1)) { ///> parse action from token (keep opcode in tmp)
  case TKN_IMM:
    _immediate(tmp);
    break; ///>> immediate words,
  case TKN_WRD:
    _nest(tmp + 2 + 3);
    break; ///>> execute colon word (user defined)
  case TKN_PRM:
    _invoke((U8)tmp);
    break; ///>> execute primitive built-in word,
  case TKN_NUM:
    PUSH(tmp);
    break;      ///>> push a number (literal) to stack top,
